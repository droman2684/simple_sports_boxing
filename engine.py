...
def _score_round(landed_a: int, landed_b: int, kd_a: int, kd_b: int, judge_bias: float=0.0) -> Tuple[int,int]:
    ...
    """
    # Base winner by landed punches with tiny bias noise
    ...
    if margin > 0.5:
        ...
        a = 10; b = 10  # even round

    # Knockdowns modify scoring (10-8 typical, 10-7 for two)
    # kd_a = knockdowns scored BY A (i.e., B went down)
    if kd_a >= 1 and a >= b:
        b = max(7, b - kd_a)  # A winning; drop B to 8 or 7
    if kd_b >= 1 and b >= a:
        a = max(7, a - kd_b)

    # If a lost the round but scored a KD, soften the loss (rare)
    if kd_a >= 1 and b > a:
        b = max(9, b)  # keep B not too high
    if kd_b >= 1 and a > b:
        a = max(9, a)
...
def simulate_fight(a: Fighter, b: Fighter, rounds: int = 12, seed: Optional[int] = None) -> Dict[str, Any]:
    ...
    # Derived modifiers (normalize to 0..1)
    ...
    fatigue_a = 0.0 # grows each round
    fatigue_b = 0.0
    ...
    for rnd in range(1, rounds+1):
        ...
        landed_a = 0; landed_b = 0
        kd_a = 0; kd_b = 0  # knockdowns THIS round (a scores kd_a, b scores kd_b)
        notes: List[str] = []

        # Each attempt: decide attacker weight by current freshness
        total_attempts = attempts_a + attempts_b
        for i in range(total_attempts):
            a_att_prob = (spd_a * (1.0 - fatigue_a) + sta_a*0.5) / ((spd_a * (1.0 - fatigue_a) + sta_a*0.5) + (spd_b * (1.0 - fatigue_b) + sta_b*0.5) + 1e-9)
            attacker_is_a = rng.random() < a_att_prob

            if attacker_is_a:
                # Hit probability factors: accuracy vs defense, plus freshness
                ...
                if rng.random() < hit_chance:
                    landed_a += 1
                    # Damage: power + randomness – opponent’s defense/guard
                ...
                if rng.random() < hit_chance:
                    ...
                    kd_prob = 0.005 + 0.020*pow_b + 0.010*max(0.0, (damage_a - 45.0)/50.0)
                    if rng.random() < kd_prob:
                        kd_b += 1
                        kd_total_a += 1
